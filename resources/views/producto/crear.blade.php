...
@section('contenido')
    <h2 class="text-center font-extrabold text-3xl mb-8 mt-4">Crear producto</h2>
    <div class="mt-4">
            ...
            @foreach ($categorias as $categoria)
                ...
                @foreach ($categoria->categoria_datos as $dato)
                    <div class="flex items-center mb-1">
                        <input name="categorias[{{ $categoria->nombre }}]" id="{{ $categoria->nombre }}-{{ $dato->nombre }}" value="{{ $dato->id }}" type="radio" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-primary-500 focus:border-primary-400 accent-primary-500 text-primary-500">

...
<div class="flex">
  <input type="password" {{ $attributes->merge(["name" => $name, "placeholder" => $placeholder, "minlength" => $minlength, "maxlength" => $maxlength, "value" => $value]) }} {{ $required ? "required" : "" }} id="{{ $name }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 grow-1 {{ $class }}" />
